...
geem2 <- function(formula, id, waves=NULL, data = parent.frame(),
                 ...
                 sandwich = TRUE, useP = TRUE, maxit = 40, tol = 0.00001){
  call <- match.call()
  ...
  if(is.null(init.beta)){
    if(any(interceptcol)){
      ...
      if (!is.nan(linkOfMean)) {init.beta[which(interceptcol)] <- linkOfMean}
    }else{
  ...
  while(!stop){
    ...
    phi.old <- phi
    if( max(abs((beta - beta.old)/(beta.old + .Machine$double.eps))) < tol ){converged <- TRUE; stop <- TRUE}
    if(count >= maxit){stop <- TRUE}
    beta.old <- beta
  }
  ...
  naiv_var <- try(solve(beta.list$hess), silent = TRUE)
  
  if (inherits(naiv_var, "try-error")) {
    message(sprintf("solve() failed: %s", attr(naiv_var, "condition")$message))
    naiv_var <- NULL
  }
  results$naiv.var <- naiv_var
...
getBlockDiag <- function(len, xvec = NULL) {
  ...
  row.vec <- col.vec <- numeric(sum(len^2))
  add.vec <- cumsum(len) - len
  
  # Ensure proper index construction
  if (K == 1) {
    index <- c(0, sum(len^2))
  } else {
    index <- c(0, cumsum(len^2))
  }
  ...
  for (i in seq_len(K)) {
    ...
    # Skip iteration if indices are invalid
    if (start_idx > end_idx) next
    
    row.vec[start_idx:end_idx] <- rep.int((1:len[i]) + add.vec[i], len[i])
    col.vec[start_idx:end_idx] <- rep((1:len[i]) + add.vec[i], each = len[i])
  }
...
updatePhi <- function(YY, mu, VarFun, p, StdErr, included, includedlen, sqrtW, useP) {
  ...
  # Compute the residuals and extract diagonal safely
  resid_matrix <- StdErr %*% included %*% sqrtW %*% Diagonal(x = YY - mu)
  resid <- Matrix::diag(as(resid_matrix, "matrix"))  # Ensure it works for sparse matrices
  # Compute phi
...
updateBeta <- function(YY, XX, beta, off, InvLinkDeriv, InvLink,
  ...
  for(i in 1:10) {
    ...
    # Ensure diagonal matrices from vectors
    dInvLinkdEta <- Diagonal(x = InvLinkDeriv(eta))
    mu <- InvLink(eta)
    StdErr <- Diagonal(x = sqrt(1 / VarFun(mu)))
    
    # Compute hessian and estimating equation
    ...
    if (is.na(d) || d < tol) {
      warning("Hessian matrix is nearly singular, non-invertible, or determinant is NA.")
      break
    }
    ...
    # Convergence check
    if (sum(abs(update)) < tol) {
      conv <- TRUE
      break
    }
  }
  
  return(list(beta = beta.new, hess = hess, converged = conv))
}
...
getSandwich = function(YY, XX, eta, id, R.alpha.inv, phi, InvLinkDeriv,
  ...
  Matrix::diag(dInvLinkdEta) <- InvLinkDeriv(eta)
  mu <- InvLink(eta)
  Matrix::diag(StdErr) <- sqrt(1/VarFun(mu))
